...
---
# github https://github.com/LucaDaniel11/EsameFondamenti.git
...
Esame Fondamenti Di Scienza Dei Dati
Analisi:
CASO DI STUDIO: NETFLIX data with IMDB scores added
Data Set contenente tutte le informazioni relativi ai video multimediali, compresi i valori assegnati dal IMDB, dei film/Tv Show della piattaforma Netflix
IMDB -> Internet Movie DataBase che è un sito web di proprietà di Amazon che gestiste informazioni su film,attori, registi, programmi televisivi etc...
...
#Tv_Show che contiene tutti i Tv Show presenti nel dataset, raggruppati per duration con relativa media del rating
Tv_Show <-
  netflix%>%
  filter(type == "TV Show")%>%
  group_by(duration)%>%
...
#Il rating dei film/Tv Show è "influenzato" dalla durata di quest'ultimo? 
...
# le due variabili ordinate servono sia per rispondere la prima domanda che per vedere sempre come è composto il dataset. Soprattutto per vedere se ci sono delle incosistenze o 
...
# eliminazione colonne inutili per il calcolo
...
#trasformazione in dataframe per essere sicuri che sia un dataframe
...
#CONCLUSIONI:
Domanda 1: Il rating dei film/Tv Show è "influenzato" dalla durata di quest'ultimo? 
--Tv Show--
Osservando il grafico possiamo notare una netta differenza tra i vari TV Show con durata delle season differenti.
Infatti vediamo che Show di poche season sono quotati molto poco rispetto agli altri. Con l'aumentare delle season vediamo un 
andamento molto stabile, quindi si potrebbe dire ottimale e che il rating è influenzato dalla durata dello show.
...
di "breve" durata (eg <50) non hanno un andamento costante,ma presentano degli sbalzi nel grafico;cosa che non succede nei film con durata oltre i 70.


Domanda 2: I film vecchi sono più quotati rispetto a quelli nuovi?
Dal grafico possiamo rilevare i rating assegnati ai film dei diversi anni: facendo un analisi più approffondita, possiamo notare che i film degli anni 50 sono 
più apprezzati rispetto agli altri.
...
Domanda 3: Su netflix quale "genere" è più seguito?
...
Considerazione possibile: i Movie sono più seguiti perchè nel dataset sono presenti molti più film rispetto ai tv show.
...
- Più trasmissioni:International Movies	1927	6.59
...
- Meno apprezzata:     Stand-Up Comedy & Talk Shows	  42    5.9
...
Da questo ne deduco che l'ambientazione in cui viene girato un film influenza la visibilità dello stesso.

Quindi:
con questa analisi ho potuto constatare che:
